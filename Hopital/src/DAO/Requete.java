...
public class Requete {
    ...
    public ArrayList<ArrayList> Requete1() {
        ...
        try {
            ResultSet result = this.connect.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
                                   ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT malade.prenom,malade.nom "
                                   + " FROM malade" + " WHERE malade.mutuelle = 'MAAF'");
    ...
    public ArrayList<ArrayList> Requete2() {
        ...
        try {
            ResultSet result = this.connect.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
                                   ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT employe.prenom,employe.nom "
                                   + " FROM infirmier NATURAL JOIN employe" + " WHERE infirmier.rotation = 'NUIT'"
        ...
        try {
            ResultSet result =
                this.connect.createStatement(
                    ...
                    "SELECT service.nom,service.batiment,employe.prenom,employe.nom,docteur.specialite"
    ...
    public ArrayList<ArrayList> Requete4() {
        ...
        try {
            ResultSet result =
                this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY).executeQuery(
                    "SELECT DISTINCT hospitalisation.lit, chambre.no_chambre, malade.nom, malade.prenom, malade.mutuelle"
                    + " FROM batiment, service, chambre, hospitalisation, malade"
                    + " WHERE hospitalisation.no_chambre = chambre.no_chambre "
                    + " AND chambre.code_service = service.code " + " AND batiment.lettre = service.batiment "
                    + " AND hospitalisation.no_malade = malade.numero" + " AND malade.mutuelle LIKE 'MN%'"
    ...
    public ArrayList<ArrayList> Requete5() {
        ...
        try {
            ResultSet result = this.connect.createStatement(
                                   ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY).executeQuery(
                                   "SELECT DISTINCT service.code, AVG( infirmier.salaire ) "
                                   + " FROM service, infirmier" + " WHERE service.code = infirmier.code_service"
                                   + " GROUP BY code_service" + " LIMIT 0 , 100");
    ...
    public ArrayList<ArrayList> Requete6() {
        ...
        try {
            ResultSet result = this.connect.createStatement(
                                   ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY).executeQuery(
                                   "SELECT DISTINCT service.code, AVG(chambre.nb_lits)"
                                   + " FROM service, batiment, chambre" + " WHERE service.batiment = 'A' "
                                   + " AND service.code = chambre.code_service" + " GROUP BY code_service ");
    ...
    public ArrayList<ArrayList> Requete7() {
        ...
        try {
            ResultSet result =
                this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY).executeQuery(
                    "SELECT malade.prenom, malade.nom,COUNT(docteur.numero) AS NB_soignant, COUNT(DISTINCT docteur.specialite) AS Nb_specialite"
                    + " FROM malade JOIN soigne ON malade.numero = soigne.no_malade JOIN docteur ON soigne.no_docteur = docteur.numero"
                    + " GROUP BY malade.numero" + " HAVING COUNT(docteur.numero)>=3");
    ...
    public ArrayList<ArrayList> Requete8() {
        ...
        try {
            ResultSet result = this.connect.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
                                   ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT A.nom,"
                                   + " (SELECT COUNT(infirmier.rotation) " + " FROM infirmier,service "
                                   + " WHERE infirmier.code_service=service.code "
                                   + " AND A.code = infirmier.code_service)/" + " (SELECT COUNT(malade.nom) "
                                   + " FROM hospitalisation,malade "
                                   + " WHERE  malade.numero = hospitalisation.no_malade "
                                   + " AND hospitalisation.code_service = A.code) AS NB_AVG_lit" + " FROM service A");
    ...
    public ArrayList<ArrayList> Requete9() {
        ...
        try {
            ResultSet result = this.connect.createStatement(
                                   ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY).executeQuery(
                                   "SELECT  employe.nom, employe.prenom, employe.numero" + " FROM docteur,employe"
    ...
    public String[] ListOfQuery() {
        String TabOfQuery[] = new String[] {
            "Requete 1", "Requete 2", "Requete 3", "Requete 4", "Requete 5", "Requete 6", "Requete 7", "Requete 8",
